"""
FastAPI REST wrapper around the local mt5linux RPyC server.

Used in MODE 2 (Dockerfile.rest). Runs INSIDE the same container as the
mt5linux server, so it talks to localhost:8001 by default.

Endpoints:
    GET  /health                                — service alive + RPyC connected
    GET  /account                               — FTMO account info
    GET  /positions                             — open positions
    GET  /symbols                               — list of available symbols
    GET  /symbol/{name}                         — symbol info + last tick
    GET  /candles?symbol=X&timeframe=M1&count=N — OHLCV bars
...
MT5_HOST = os.environ.get("MT5_RPYC_HOST", "127.0.0.1")
